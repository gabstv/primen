...
func (t *TileSet) Update(ctx UpdateCtx, tr *Transform) {
	if t.db == nil {
		t.isValid = false
		return
	}
...
func (t *TileSet) Draw(ctx DrawCtx, tr *Transform) {
	if t.disabled {
		return
	}
	g := tr.m
...
//go:generate ecsgen -n TileSet -p core -o tileset_component.go --component-tpl --vars "UUID=775FFA75-9F2F-423A-A905-D48E4D562AE8" --vars "Setup=c.onCompSetup()"
