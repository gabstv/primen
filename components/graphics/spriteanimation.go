...
//go:generate ecsgen -n SpriteAnimation -p graphics -o spriteanimation_component.go --component-tpl --vars "UUID=5A056275-C47D-44D2-994C-BD0AF107870C"

//go:generate ecsgen -n SpriteAnimation -p graphics -o spriteanimation_system.go --system-tpl --vars "Priority=12" --vars "EntityAdded=s.onEntityAdded(e)" --vars "EntityRemoved=s.onEntityRemoved(e)" --vars "UUID=FFD3127E-6066-4561-8B2A-E1B59EBE489C" --components "Sprite" --components "SpriteAnimation"
...
func (s *SpriteAnimationSystem) Update(ctx core.UpdateCtx) {
	dt := ctx.DT()
	globalfps := core.Nonzeroval(ctx.TPS(), 60)
	for _, v := range s.V().Matches() {
		...
		localfps := core.Nonzeroval(clip.GetFPS(), v.SpriteAnimation.fps, globalfps)
		...
		if v.SpriteAnimation.t >= 1 {
			...
			if nextframe >= clip.GetFrameCount() {
				...
				case AnimLoop:
					v.SpriteAnimation.t = core.Clamp(v.SpriteAnimation.t-1, 0, 1)
					v.SpriteAnimation.activeFrame = 0
					v.SpriteAnimation.trySwapImage(clip, 0, v.Sprite)
				case AnimPingPong:
					v.SpriteAnimation.t = core.Clamp(v.SpriteAnimation.t-1, 0, 1)
					v.SpriteAnimation.activeFrame = core.Intmax(clip.GetFrameCount()-2, 0)
